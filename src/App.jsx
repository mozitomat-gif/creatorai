...
const DRIVE_SCOPE = "https://www.googleapis.com/auth/drive https://www.googleapis.com/auth/drive.file https://www.googleapis.com/auth/drive.readonly https://www.googleapis.com/auth/drive.metadata.readonly";
...
export default function App() {
  ...
  async function fetchDriveFiles(token) {
    ...
    setError(null);
    try {
      // First find the "CreatorAI" folder
      const folderRes = await fetch(
        `https://www.googleapis.com/drive/v3/files?q=name='CreatorAI' and mimeType='application/vnd.google-apps.folder' and trashed=false&fields=files(id,name)`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const folderData = await folderRes.json();
      
      let query = "trashed=false and (mimeType contains 'video' or name contains '.mp4' or name contains '.mov' or name contains '.avi' or name contains '.mkv')";
      
      // If folder found, search inside it
      ...
      const res = await fetch(
        `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(query)}&fields=files(id,name,mimeType,createdTime,size,thumbnailLink)&orderBy=createdTime desc&pageSize=50`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      setDriveFiles(files);
      if (files.length === 0) setError("Aucun fichier trouvé dans le dossier CreatorAI. Vérifie que le dossier existe et contient des vidéos.");
      setVideoQueue(prev => {
        const existingIds = prev.map(v => v.id);
        const newFiles = files.filter(f => !existingIds.includes(f.id));
        return [...prev, ...newFiles];
      });
    } catch(e) { setError("Erreur chargement Drive : " + e.message); }
